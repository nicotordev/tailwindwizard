...
export function CartSheet() {
  ...
  const subtotal = items.reduce((sum, item) => {
    const price = item.block?.price 
      ? (typeof item.block.price === 'string' ? parseFloat(item.block.price) : item.block.price) 
      : 0;
    return sum + price;
  }, 0);
                            ...
                            <div key={item.id} className="flex gap-4 p-1">
                                <div className="relative size-20 rounded-xl overflow-hidden bg-muted/20 border border-border/50 shrink-0">
                                    {item.block?.screenshot || item.block?.previews?.[0]?.url ? (
                                        <Image
                                            src={item.block.screenshot || item.block.previews![0].url}
                                ...
                                <div className="flex-1 min-w-0 flex flex-col justify-between">
                                    <div>
                                        <h4 className="font-semibold truncate">{item.block?.title}</h4>
                                        <p className="text-xs text-muted-foreground">
                                            {item.licenseType === "PERSONAL" ? "Personal License" : item.licenseType}
                                        </p>
                                    </div>
                                    <div className="flex items-center justify-between mt-2">
                                        <div className="font-mono font-medium">
                                            <Money amount={item.block?.price} currency={item.block?.currency} />
                                        </div>
                                        <Button
                                            variant="ghost"
                                            size="icon"
            ...
            <div className="space-y-4 pt-4 border-t">
                <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium"><Money amount={subtotal} /></span>
                    </div>
                    <Separator />
